...
list($_l, $_g) = Nette\Latte\Macros\CoreMacros::initRuntime($template, 'fur6c26gzo')
;
// prolog Nette\Latte\Macros\UIMacros
//
// block content
//
if (!function_exists($_l->blocks['content'][] = '_lb650853febc_content')) { function _lb650853febc_content($_l, $_args) { extract($_args)
...
// block _header
//
if (!function_exists($_l->blocks['_header'][] = '_lb8e23da1a77__header')) { function _lb8e23da1a77__header($_l, $_args) { extract($_args); $_control->validateControl('header')
...
// block _flashes
//
if (!function_exists($_l->blocks['_flashes'][] = '_lb1b7f96c949__flashes')) { function _lb1b7f96c949__flashes($_l, $_args) { extract($_args); $_control->validateControl('flashes')
?>    <div id="flashes">
...
// block _windows
//
if (!function_exists($_l->blocks['_windows'][] = '_lb6c4b88f8f0__windows')) { function _lb6c4b88f8f0__windows($_l, $_args) { extract($_args); $_control->validateControl('windows')
?>    <div id="windows">
<?php if ($page['loginForm']): Nette\Latte\Macros\CoreMacros::includeTemplate(''._window('login'), get_defined_vars(), $_l->templates['fur6c26gzo'])->render() ;Nette\Latte\Macros\UIMacros::callBlock($_l, 'loginWindow', array('loginForm' => $page['loginForm']) + $template->getParameters()) ;endif ?>
    </div>
<?php
}}
...
// block _page
//
if (!function_exists($_l->blocks['_page'][] = '_lbd67c649b1d__page')) { function _lbd67c649b1d__page($_l, $_args) { extract($_args); $_control->validateControl('page')
...
<?php Nette\Latte\Macros\CoreMacros::includeTemplate(''.(\Kate\Main\Loader::MODULES_DIR).S.$content['moduleLabel'].S.'default.latte', get_defined_vars(), $_l->templates['fur6c26gzo'])->render() ;$contentBlock =  'contentModule'.$content['moduleLabel'] ;Nette\Latte\Macros\UIMacros::callBlock($_l, $contentBlock, array('moduleContent' => $content['moduleContent'], 'content' => $content) + $template->getParameters()) ?>
                                                </div>
                                            </div>
                                        </div>
...
// block head
//
if (!function_exists($_l->blocks['head'][] = '_lb8ee03c3dd6_head')) { function _lb8ee03c3dd6_head($_l, $_args) { extract($_args)
?><style type="text/css">
    #body {
